					...
					struct rockchip_panel_cmds *cmds)
{
	...
	for (i = 0; i < cmds->cmd_cnt; i++) {
		...
		//printf("mipi-panel-%s::%d::data_type is %#x,%#x\n",__func__,__LINE__,header->data_type,desc->payload[0]);
...
static void panel_simple_prepare(struct rockchip_panel *panel)
{
	...
	printf("mipi panel reset\n");
	if (dsi && !dev_read_bool(dsi->dev, "is3587")) {
                ret = mipi_dsi_dcs_get_power_mode(dsi, &mode);
                printf("mipi-panel---err=%d, mode=%02x. [err:0 comunication is ok, mode: 08=>device off 9c=>device on]\n", ret, mode);
        }
